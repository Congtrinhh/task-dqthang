...
async function sendRequest(data) {
	...
	const myRequest = new Request("https://dti.hcm.edu.vn/sync-data/lms/sendEquipmentInfo", {
		method: "POST",
		headers: headers,
...
function saveJsonFile(rawData, fileName = "Kết quả.json") {
...
//check xem cors mode có được bật không
window.addEventListener("DOMContentLoaded", () => {
	const iframe = document.querySelector("iframe");
	iframe.addEventListener("load", () => {
		const canAccessIframeDocument = !!iframe.contentDocument;
		document
			.querySelector(canAccessIframeDocument ? ".security-disabled" : ".security-enabled")
...
<template>
	...
	<p>This test attempts to access the inner contents of a cross-origin iframe, which is normally disallowed.</p>
	<p class="security-enabled hidden">
		Web security is enabled. The cross-origin iframe document could not be accessed.(Không thể gọi api khi mode
		security đang bật)
	</p>
	<p class="security-disabled hidden">
		Web security is disabled. The cross-origin iframe document was successfully accessed.
	</p>
	<iframe class="hidden"> Iframes are not supported. </iframe>

	<button @click="sendAllRequests" style="border: 1px solid">Bấm để gửi tất cả</button>
	<!-- <button @click="sendOneRequest" style="border: 1px solid">Bấm để gửi thử 1</button> -->
	<!-- <button @click="saveJsonFile" style="border: 1px solid">Bấm để ghi file json</button> -->
	<h2>{{ msg }}</h2>
